...
Toolbar::Toolbar() : m_gridlayout()
{
	...
	connect(bnFill, &QPushButton::pressed, this, [&](){
		emit toolChanged(Toolbar::ToolMode::FILL);
	});
	connect(bnPencil, &QPushButton::pressed, this, [&](){
	  emit toolChanged(Toolbar::ToolMode::PENCIL);
	});
	connect(bnPaintbrush, &QPushButton::pressed, this, [&](){
	  emit toolChanged(Toolbar::ToolMode::PAINTBRUSH);
	});
